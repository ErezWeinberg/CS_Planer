...
function PlannerApp({ courses, trackDef, availableYears }: { courses: Map<string, SapCourse>; trackDef: TrackDefinition; availableYears: number[] }) {
  ...
  const DARK_MODE_KEY = 'cs-dark-mode';
  ...
  const handleCourseAdded = useCallback((courseName: string, semesterLabel: string) => {
    ...
    setToast({ message: `${courseName} ${t('addedTo')} ${semesterLabel}`, visible: true });
  ...
  useEffect(() => {
    ...
    const handleSaveError = (error: unknown, onSuccess?: () => void) => {
      ...
      if (isRetryableSyncError(error)) {
        setSyncStatus('error');
        setSyncErrorMessage(`${message}. ${t('autoRetry')}`);
        ...
        >
          {shareMode.isShareReview && (
            <>
              <span className="font-semibold">
                {t('shareReviewTitle')}{shareMode.isNewShareReview ? ' · ' + t('newUpdate') : ''}
              </span>
              <span>
                {t('shareReviewDesc')}
              </span>
          ...
          <span>
            {shareMode.canEdit
              ? t('shareEditWarning')
              : t('shareViewWarning')}
            {shareMode.share.ownerEmail && ` {t('sharedBy')} ${shareMode.share.ownerEmail}.`}
          </span>
          {shareMode.canEdit && shareSaveStatus !== 'idle' && (
            <span className={shareSaveStatus === 'error' ? 'text-red-600' : shareSaveStatus === 'saved' ? 'text-emerald-700' : 'text-amber-600'}>
              {shareSaveStatus === 'saving' ? t('saving') : shareSaveStatus === 'saved' ? `✓ ${t('saved')}` : t('saveError')}
            </span>
          )}
          {shareMode.canEdit && !shareMode.isOwner && (
            <span className="text-amber-600">
              {t('workOnSeparateCopy')}{' '}
              <a href={window.location.origin} className="underline hover:text-amber-900">{t('openYourPlanner')}</a>.
            </span>
          )}
            </>
          )}
          <a href={window.location.origin} className="underline font-medium hover:opacity-80">{t('goToMyPlan')}</a>
        </div>
      )}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2 justify-end">
              ...
              <button
                ...
                className="text-sm border px-3 py-1.5 rounded-lg transition-colors bg-blue-600/20 hover:bg-blue-600/40"
                style={{ color: 'rgba(147,197,253,1)', borderColor: 'rgba(147,197,253,0.3)' }}
                title={t("systemGuide")}
              >
                <span>💡</span><span className="hidden sm:inline"> {t('guide')}</span>
              </button>
              ...
              {(!shareMode || shareMode.isOwner) && (
                <button
                  ...
                  title={pendingShareUpdates.length > 0
                    ? `${t('pendingUpdates1')}${pendingShareUpdates.length}${t('pendingUpdates2')}`
                    : t('exportShareImport')}
                >
                  <span>⇪</span><span className="hidden sm:inline"> {t('shareImport')}</span>
                  {pendingShareUpdates.length > 0 && (
                    <span
                      ...
                      aria-label={`${pendingShareUpdates.length} ${t('pendingUpdatesLabel')}`}
                    >
              ...
              <button
                ...
                style={{ color: 'rgba(255,255,255,0.75)', borderColor: 'rgba(255,255,255,0.2)' }}
                title={darkMode === 'dark' ? t('darkModeToLight') : darkMode === 'light' ? t('lightModeToAuto') : t('autoModeToDark')}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded-xl shadow border border-red-200 max-w-md text-center">
          <p className="text-red-600 font-semibold mb-2">{t('errorLoading')}</p>
